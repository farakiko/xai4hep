...
class LRP:
    ...
    """
    LRP class that introduces useful helper functions defined on a PyTorch model,
    and an explain method that runs layerwise-relevance propagation.

    Currently supports:
        a. Linear, activation, and BatchNorm1d layers in the model
        b. skip connections provided that they are input_features skip connections and
            that they are defined in the following order torch.cat[(input_features, ...)]
    ...
    def explain(self, input, neuron_to_explain):
        ...
        Then, it attempts to explain the whole model by looping over the layers in the model and
        invoking the explain_single_layer function.
    ...
    def eps_rule(self, layer, x, Rscores_old, neuron_to_explain):
        ...
        The computation is composed of 3 steps:
            ...
            c. matrix multiply W and the scaled Rscores, then elementwise multiply with the activations to update the Rscores
